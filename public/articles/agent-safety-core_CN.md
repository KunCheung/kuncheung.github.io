...
这个例子已经足够说明，Agent 的核心风险在于它会基于不完整信息持续做判断，并把这些判断沿着读信息、调工具、改状态、做动作的链条一步步传导。
...
所以，关键是“人什么时候出现”，如何把人的注意力留在最值得介入的地方。
...
规则最核心的价值不在于提升模型的平均判断质量，而在于即使模型判断错了，系统也不会因此立刻冲出安全范围。
...
这两点其实也再次说明：规则的本质是在模型决策出错时，仍然保证系统不轻易冲出合理范围。
...
一旦承认这一点，防御思路就必须跟着改变。问题不再是“如何确保模型永远不被影响”，而是“当模型已经被影响时，系统如何不让它造成实质性破坏”。这也是为什么，面对 Prompt Injection，更合理的原则是“假设失陷”，或者说“零信任”。微软近来的安全建议同样强调，要把间接注入、越界调用和高影响误操作，当作需要用系统级控制来约束的执行风险。
...
这样一来，即使 Prompt Injection 成功，模型下达了危险指令，系统层也会直接拒绝执行。真正被保护的是系统的“行动能力”。

所以，Prompt Injection 真正证明的是：既然模型内部的概率判断注定会被不可信环境持续影响，那么安全就不能主要寄托于模型内部的自我辨别能力，而必须依赖模型之外的确定性结构。
...
而一旦把它们放进同一个框架里，你就会发现，值得处理的是那条贯穿始终的风险链：模型如何理解输入、系统如何区分信息来源、哪些信息可以进入决策、决策如何转成执行、执行范围由谁来收紧、出错之后如何限制后果、保留审计并实现纠偏。

这也是为什么，我更愿意把 Agent 安全理解为“治理”而不是“防御”。因为它面对的是一整套运行秩序的问题，而非一个单点漏洞。
...
第三，让安全机制覆盖长链任务。Agent 的很多风险是沿着多步计划、状态更新和工具调用一路累积、放大。因此，真正成熟的 Agent 安全，不应只是静态防御，而应具备过程治理能力。

> 安全的目标，从来不是让模型变成一个永不犯错的理性体，而是在承认其不确定性的前提下，为它设计一个不会轻易失控的世界。

如果重新回看整篇文章，会发现核心问题始终没有变：为什么大模型 / Agent 的安全问题，比传统内容安全复杂得多？

根本原因在于，模型的角色变了。它不再只是一个生成文本的工具，而正在成为一个在开放环境中持续理解、判断并行动的决策体。也正因为如此，Agent 安全不能再停留在针对单点攻击的补丁式防御，也不能寄希望于训练出一个永不犯错的模型。

更现实的目标是：承认模型具有概率性和不确定性，但让系统整体仍然可靠。构建一套适配概率性智能的运行机制：信息源需要分层，权限需要最小化，高风险动作需要人或规则介入，错误也必须被限制在可控范围内。
